...
# Starting values for lower bound, upper bound and guess.
lower = balance / 12.0
upper = (balance * (1 + monthlyInterestRate)**12) / 12.0
guess = (lower + upper) / 2.0

def oneMonth(balance, guess):
    monthlyBalance = balance - guess  
    updatedBalance = monthlyBalance + (monthlyInterestRate * monthlyBalance)
    return updatedBalance
    
def getYear(startBalance):
    newBalance = oneMonth(startBalance, guess)
    for months in range(1,12):
        newBalance = oneMonth(newBalance, guess)
    return newBalance

# Handy little helper function
def computeGuess(low, up):
    return (low + up) / 2.0 

# Main computational logic is here. We keep adjusting upper and lower bounds 
# until we get a value for guess that satisfies our accuracy constraint. 
while abs(getYear(balance)) > 0.01:
    if getYear(balance) > 0.01:
        lower = guess
    else:
        upper = guess
    guess = computeGuess(lower, upper)
