...
WORDLIST_FILENAME = "C:/Users/Administrator/Desktop/Repo/6.00.1x Files/assignments/week 4/words.txt"
...
def getWordScore(word, n):
    ...
    if not multiplier:
        ...
        points = 0
        for x in word:
            points += SCRABBLE_LETTER_VALUES[x]
        points *= multiplier
        if multiplier == n:
            points += 50
    return points
...
def isValidWord(word, hand, wordList):
    ...
    if word == '' or word not in wordList:
        ...
        dict_check = dict(hand)
        for char in word:
            if not dict_check.get(char, 0):
                return False
            else:
                dict_check[char] -= 1           
    return True
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print "%s earned %d points. Total: %d points" % \
                       (user_input, getWordScore(user_input, n), score) 
