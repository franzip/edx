...
class Admin::ContentController < Admin::BaseController
  ...
  def merge
    ...
    if @user.profile.label != "admin"
      flash[:error] = _("You don't have the permissions to do that!")
    elsif target_id.blank?
      flash[:error] = "Please supply an article ID!"
    elsif target_id == @current_article_id
      flash[:error] = "Cannot marge an article with itself!"
    else
      if Article.where(:id => target_id).blank?
        flash[:error] = "Cannot find the specified article!"
      else
        @current_article = Article.find(@current_article_id)
        @current_article.merge_with(target_id)
        flash[:notice] = _("Articles were successfully merged!")
      end
    end
    redirect_to :action => :edit, :id => @current_article_id
  end
